...
//    {AnaInSlavePos,  Beckhoff_EL3102, 0x3101, 2, &off_ana_in_value},
		{ AnaOutSlavePos, Beckhoff_EL4112, 0x3001, 1, &off_ana_out },
//    {DigOutSlavePos, Beckhoff_EL2032, 0x3001, 1, &off_dig_out},
		{ } };

static unsigned int counter = 0;
static unsigned int blink = 0;
static double rollDouble = 0.0;
double * rollDoublePtr = &rollDouble;
...
static ec_pdo_entry_info_t el4112_pdo_entries[] = { { 0x3001, 1, 16 }, // channel 1 value2
		{ 0x3002, 1, 16 }, // channel 2 value
		};

static ec_pdo_info_t el4112_pdos[] = { { 0x1600, 1, el4112_pdo_entries }, {
		0x1601, 1, el4112_pdo_entries + 1 } };

static ec_sync_info_t el4112_syncs[] = { { 2, EC_DIR_OUTPUT, 2, el4112_pdos }, {
		3, EC_DIR_INPUT }, { 0xff } };
...
//    EC_WRITE_U8(domain1_pd + off_dig_out, blink ? 0x06 : 0x09);
	EC_WRITE_U16(domain1_pd + off_ana_out, rollDouble>0.0 ? 0x8CCC : 0x7334);
//	EC_WRITE_U16(domain1_pd + off_ana_out, readIMUData()>0 ? 0x8CCC : 0x7334);
#endif
...
int main(int argc, char **argv) {
	...
	pthread_create(&pth,NULL,readIMUData,rollDoublePtr);
...
//    }
	...
	if (!(sc = ecrt_master_slave_config(master, AnaOutSlavePos, Beckhoff_EL4112))) {
		fprintf(stderr, "Failed to get slave configuration.\n");
		return -1;
	}

	if (ecrt_slave_config_pdos(sc, EC_END, el4112_syncs)) {
		fprintf(stderr, "Failed to configure PDOs.\n");
		return -1;
	}
